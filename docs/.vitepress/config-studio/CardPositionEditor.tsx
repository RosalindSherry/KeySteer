  ...
  setup(props, { emit }) {
    ...
    return () => {
      ...
      return <div class="ks-position-editor">
        <div class="ks-position-heading"><strong>可视化定位 · {props.reference === 'screen' ? '当前屏幕' : '窗口'}</strong>
          <div class="ks-position-tools">
            <button type="button" aria-pressed={!range.value} onClick={() => { range.value = false }}>定位点</button>
            <button type="button" aria-pressed={range.value} onClick={() => { range.value = true }}>绘制范围</button>
          </div>
        </div>
        <p>{range.value ? '在示意图上拖出定位范围；点击可收为一个点。' : '点击或拖动，设置卡片的首选中心点。'} 实际卡片仍会避让编号和帮助面板。</p>
        <div class={{ 'ks-position-canvas': true, dragging: dragging.value }} aria-label="位置示意图，支持鼠标或触摸拖动；也可使用下方边距滑块"
          onPointerdown={event => {
          ...
          <span class="ks-position-origin">{props.reference === 'screen' ? '当前屏幕可用区域' : '窗口 / 布局区域'}</span>
        ...
        <div class="ks-position-presets">{presets.map(([name, value]) => <button type="button" onClick={() => emit('change', [...value])}>{name}</button>)}</div>
        <div class="ks-position-sliders">{['上', '右', '下', '左'].map((name, index) => <label>
          <span>{name} <output>{props.position[index]}</output></span>
          <input type="range" aria-label={`${name}边距百分比`} min="0" max="100" step="0.1" value={insets.value[index] * 100} onInput={event => {
