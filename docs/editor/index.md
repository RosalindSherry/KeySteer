...
在模拟器中选择 Window 或 A/E/R/T 子模式，即可在卡片编辑区实时预览外观。颜色选择器和透明度滑块支持背景、编号、程序名、标题和边框；字号、字体、加粗、圆角、边框宽度及间距可直接修改。位置支持点选／拖动定位点、拖出范围、四边滑块和居中／顶部横排等预设。所有修改同步到模拟器和下载的 TOML，各项可单独重置。位置范围仍是首选位置，实际卡片继续避让其他标记。
