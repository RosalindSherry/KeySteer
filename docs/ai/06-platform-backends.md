...
Tabs 跟随保留独立顶层窗口和独立标签栏，不重新嵌入外部应用。Windows MOVESIZESTART/END 在后端记录原生手势：期间直接定位/缩放自有标签栏，不反向校正应用几何；结束后按当前快照执行一次必要的占位校正。隐藏成员仅在激活时同步。库存枚举每 8 项让出原生消息处理并检查取消，不为跟随新增轮询。macOS AX 几何通知与内容通知分离，主线程 mailbox 支持逐组更新；事件驱动优化与 macOS 原生验收见下文及构建测试专题。
...
Quick 分屏和 Editor 布局仍复用共享请求与约束算法。AX 设置矩形采用先定位、再尺寸写入、有限回读和必要的一次尺寸重试，避免旧位置限制新尺寸或将异步旧读数当作最小尺寸。受应用尺寸约束的分屏保持中心并限制在目标工作区；普通位移不增加工作区裁剪。所有回读只在窗口 worker 内执行，每次等待至多 250ms，检查取消。

macOS 单独记录最大化时实际接受的矩形及还原矩形；标签栏头部占位通过 `tab_fit_frame` 保留这两者，普通分屏／编辑清除最大化状态。F 与 Windows 一样是最大化→最小化→还原三态循环，撤销恢复同步还原这些状态。逐窗口最小化确认有 1 秒上限；公开 AX 的系统最小化动画仍与 Windows 透明度收起不同。

AppKit 标签栏在普通窗口层按公开 Quartz window number 排在活动应用窗口上方，不再全局置顶。每个标签保留最小 100 点宽度，纵／横滚动使用同一水平视口，切换活动成员自动滚入视口，标题更新保留用户滚动。标签按钮处理点击、拖动阈值和插入提示，单标签拖放／组号整组拖放提交共享 `TabDrop`，空白落点不提交；关闭栏仍只解散组。仅移动复用所有控件，标题更新不重建成员控件。

窗口操作的 AX 超时为 250ms，独立于扫描节点的短超时；`Unconfirmed` 写入仍通过有限回读确认，明确拒绝继续返回错误。AXRaise 不支持时尝试 AXMain／AXFocused，并核对实际焦点。无 Quartz 标题的完全重叠窗口仅在可见记录覆盖整个候选集合时整批纳入，无法确定窗口号时不猜测标签栏 owner；不完整集合仍过滤，以免跨 Space 操作。

macOS 应用音频在没有 Core Audio 进程时保存内存偏好并显示待生效状态；运行中暂时失去音频进程时停止 Tap／引擎但保留偏好，进程重新出现后重建。应用退出或输出丢失仍删除偏好。重建失败记录一次并释放路由，避免持续重试权限失败；回调缓冲隔离后不重用该路由。

刚写入的窗口在 AX 和 Quartz 间存在短暂几何差异；枚举对未匹配候选刷新一次 Quartz，仅在最近 250ms 写入的窗口仍处于同步期时有限等待重读，检查取消。最小化目标已达成时不重复写；动画期间丢失的同状态请求最多重试一次，总确认期限仍为 1 秒。

## macOS Tabs 事件驱动跟随

窗口 worker 的 `event_waker`／`wait_for_events` 接入 `native::RunLoopWake`：非阻塞 Unix socket pair 注册为 CFFileDescriptor run-loop source，AX observer 与 mailbox/UI 消息共同唤醒，信号先于等待也不会丢失。无刷新周期、拖动计时器或 20ms 轮询；无工作时阻塞等待事件。清空窗口观察保留唤醒源，销毁时先移除 CF source、invalidate descriptor，再关闭 FD。AX 非阻塞 pump 也排空并重新启用唤醒 descriptor，避免消费 callback 后丢失后续唤醒。
