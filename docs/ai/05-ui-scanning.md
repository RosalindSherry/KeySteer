...
owner 的 quarantine，本进程后续禁用视觉扫描，不能阻塞 Engine 或静默 detach。系统 OCR
继续在当前进程的 generation-scoped provider 中运行，不保留进程外系统 OCR helper。
...
7. Partial 保持立即显示。UIA/Vision 来源先合并、去重并分配 Hint，视觉层只接受这份统一标签
   ...
   Partial 会用新的完整合并集合更新计划。Hint 前缀、Backspace 和名称筛选每次改变可见集合后
   也从完整 Hint 索引重建冲突图与层序。扫描结束后每次新按下置顶下一个非默认全局层，松开
...
- AX 和 Vision 共用纯计数的流式发布器以及跨平台 `SpatialIndex`；Hybrid 在 scoped thread
  中并行 AX，同时当前 worker 执行 Vision。两路先按共享矩形规则去重，合计首批 24 项立即
  发布，之后在累计数量达到 48、96、192……时发布，终态无条件刷新剩余项。这里不使用
  时间间隔，因此不依赖显示刷新率或机器速度。
...
- AX 目标必须位于 focused window bounds 与请求屏幕 bounds 的交集内。`visible_only`
  扫描中的表格、列表、outline 和 scroll area 优先遍历
  `AXVisibleRows`/`AXVisibleChildren`，仅在应用不支持这些标准可见集合时回退
  `AXChildren`，避免为滚动区域中的离屏元素生成标签。
